...
import type { LtiEngagementSummary, LtiSessionContext } from '@/types/lti'
...
export function AdvisingToolPage() {
  ...
  const selectedStudent =
    context.roster.find((student) => student.studentId === selectedStudentId) ??
    context.roster[0] ??
    null
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-col gap-4 border-b border-neutral-200 pb-5 md:flex-row md:items-end md:justify-between">
          ...
          <Badge tone={context.isMapped ? 'success' : 'warning'}>
            {context.isMapped ? 'Mapped SIS context' : 'Unmapped launch'}
          </Badge>
        </header>

        {!context.isMapped ? (
          <Alert tone="warning" title="Limited launch context">
            Moodle launch validation succeeded, but the Moodle user or course is
            not mapped to SIS records yet. Run Lane A provisioning and confirm
            `MoodleUserMap` and `MoodleCourseMap` before using roster data.
          </Alert>
        ) : null}
          ...
          <Card className="md:col-span-2">
            ...
            {context.section ? (
              <dl className="grid gap-3 text-sm sm:grid-cols-2">
                <Info
                  label="Course"
                  value={`${context.section.courseCode} ${context.section.courseTitle}`}
                />
                <Info label="Section" value={context.section.sectionCode} />
                <Info
                  label="Term"
                  value={`${context.section.semester} ${context.section.academicYear}`}
                />
                <Info label="Faculty" value={context.section.faculty} />
              </dl>
            ) : (
              <p className="text-sm text-neutral-600">
                {context.launch.context.title ??
                  'No mapped SIS section available.'}
              </p>
            )}
          </Card>
        ...
        <Card>
          ...
          {context.roster.length > 0 ? (
                    ...
                    <tr key={student.studentId}>
                      <td className="px-3 py-3 font-medium text-neutral-900">
                        <button
                          ...
                          className="text-left font-medium text-info underline-offset-4 hover:underline focus:outline-none focus:ring-2 focus:ring-info focus:ring-offset-2"
                          onClick={() =>
            ...
            <p className="text-sm text-neutral-600">
              No SIS roster is available for this launch context.
            </p>
          )}
          {selectedStudent ? (
            <section className="mt-5 grid gap-4 rounded-md border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-2">
              <div>
                <h2 className="text-base font-semibold text-neutral-900">
                  Selected student
                </h2>
                <dl className="mt-3 grid gap-3 text-sm">
                  <Info label="Student" value={selectedStudent.fullName} />
                  <Info
              ...
              <EngagementSummary engagement={selectedStudent.engagement} />
            </section>
          ) : null}
        </Card>
      </div>
    ...
    <div>
      ...
      {engagement ? (
        <dl className="mt-3 grid gap-3 text-sm">
          <Info
            label="Collected"
            value={formatDateTime(engagement.collectedAt)}
          />
          <Info
            label="Last Moodle access"
            value={formatDateTime(engagement.moodleLastAccessAt)}
          />
          <Info
            label="Course last access"
            value={formatDateTime(engagement.moodleCourseLastAccessAt)}
          />
          <Info
            label="Assignments"
            value={formatMetric(engagement.assignmentSubmissionCount)}
          />
          <Info
            label="Quizzes"
            value={formatMetric(engagement.quizAttemptCount)}
          />
          <Info
            label="Forum posts"
            value={formatMetric(engagement.forumPostCount)}
          />
        </dl>
      ) : (
        <p className="mt-3 text-sm text-neutral-600">
          No Moodle engagement snapshot has been collected for this student yet.
        </p>
      )}
    </div>
  )
